...
##############
@register('pre-image-folder')
class PreImageFolder(Dataset):
    def __init__(self, path,  split_file=None, split_key=None, first_k=None, size=None,
                 repeat=1, cache='none', mask=False):
        self.repeat = repeat
        self.cache = cache
...
@register('pre-image-folders')
class PreImageFolders(Dataset):

    def __init__(self, root_path_p, **kwargs):
        self.dataset_p = PreImageFolder(root_path_p, **kwargs)
        self.dataset_d = ImageFolder(root_path_p, **kwargs)
    ...
    def __getitem__(self, idx):
        ...
        return self.dataset_d[idx], self.dataset_p[idx]
